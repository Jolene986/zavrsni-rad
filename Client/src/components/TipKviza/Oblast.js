...
class Oblast extends Component {
  ...
  render() {
    return (
      <Pomocna>
        <Modal
          show={this.state.pokaziOpciju}
          modalClosed={this.odustani}
          className={[classes.Modal, classes.Home].join(" ")}
        >
          <Tezina setTezina={this.props.tezina} />
          <Nadimak setujNadimak={this.props.snadimak} />
          <button className={classa.Button} onClick={this.zapocniKviz}>
            Započni kviz
          </button>
        </Modal>

        <div
          className={classa.Div}
          id={this.props.id}
          onClick={this.pokaziOpciju}
        >
          {this.props.oblast}
        </div>
      </Pomocna>
